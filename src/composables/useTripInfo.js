...
export default function useTripInfo() {
	...
	const getDestinationName = (pointIndex) => {
		return getDestinationById.value(sortedPoints.value[pointIndex].destination).name;
	}
	...
	const tripRoute = computed(() => {
		if (sortedPoints.value.length <= MAX_POINTS_SHOW) {
			...
			const point2 = sortedPoints.value[1] ? ` \u2014 ${getDestinationName(1)}` : '';
			const point3 = sortedPoints.value[2] ? ` \u2014 ${getDestinationName(2)}` : '';
